...
export const FreelancerProfile = () => {
    ...
    const fetchUserData = async () => {
        ...
        // For now, we'll just fetch the first freelancer user.
        try {
            ...
            if (response.ok) {
                const freelancerUser = data.records.find(u => u.role === 'freelancer');
                            ...
                            {isEditing ? (
                                <div className="profile-form" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                    ...
                                    <div className="input-group">
                                        ...
                                        <input type="number" name="rate" value={user.rate} onChange={handleInputChange} />
                                    </div>
                                    <div className="input-group" style={{ gridColumn: '1 / 3' }}>
                                        <label>Skills (comma separated)</label>
                                        <input type="text" name="skills" defaultValue={user.skills.join(', ')} onChange={handleSkillsChange} />
                                    </div>
                                ...
                                <div className="profile-info" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                    ...
                                    <div><strong>Hourly Rate:</strong><p>R {user.rate}/hr</p></div>
                                    <div><strong>Skills:</strong><div className="skills-container" style={{ marginTop: '0.5rem' }}>{user.skills.map(s => <span key={s} className="skill-tag">{s}</span>)}</div></div>
                                </div>
                            )}
                        </div>
                    </div>
