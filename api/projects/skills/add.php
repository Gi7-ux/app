...
try {
    ...
    $sql = "INSERT INTO project_skills (project_id, skill_name) VALUES (:project_id, :skill_name)";
    $stmt = $db->prepare($sql);
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(array("message" => "Unable to add skill."));
    }
    ...
    if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
        ...
        echo json_encode(array("message" => "Skill already exists for this project."));
    } else {
        http_response_code(503);
        echo json_encode(array("message" => "Unable to add skill.", "error" => $e->getMessage()));
    }
}
?>